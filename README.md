...
2. `detected_language` & `detected_script`: Computed via our custom heuristic engine.
3. `source_file`: Ensuring immutable data provenance.
4. `confidence_score`: A master reliability metric aggregated from word-level probabilities, allowing Task 4 to automatically threshold low-quality chunks before generating vector embeddings.

## Future Work (Phase 2: Neural Roadmap)
